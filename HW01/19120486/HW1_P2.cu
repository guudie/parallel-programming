		...
		uchar3 * outPixels)
{
	...
	if(r < height && c < width) {
		int i = r * width + c;
		float x = 0, y = 0, z = 0;
		for(int fri = 0; fri < filterWidth; fri++) {
			for(int fci = 0; fci < filterWidth; fci++) {
				int f_r = r - filterWidth / 2 + fri;
				int f_c = c - filterWidth / 2 + fci;
				f_r = (f_r < 0) ? 0 : ((f_r >= height) ? height - 1 : f_r);
				f_c = (f_c < 0) ? 0 : ((f_c >= width) ? width - 1 : f_c);
				int f_i = f_r * width + f_c;
				x += inPixels[f_i].x * filter[fri * filterWidth + fci];
				y += inPixels[f_i].y * filter[fri * filterWidth + fci];
				z += inPixels[f_i].z * filter[fri * filterWidth + fci];
			}
		}
		outPixels[i].x = x;
		outPixels[i].y = y;
